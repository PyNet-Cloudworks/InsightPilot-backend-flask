...
def extract_keyframes(video_path, output_dir="static/video_frames", cleanup_input=True, cleanup_frames=False):
    os.makedirs(output_dir, exist_ok=True)
    base_name = os.path.basename(video_path)
    prefix = datetime.now().strftime('%Y%m%d_%H%M%S_') + base_name
    ...
    while True:
        ...
        if frame_count % 90 == 0:
            ...
            if cleanup_frames and os.path.exists(raw_path):
                os.remove(raw_path)
    ...
    # Optional: delete the original video after processing
    if cleanup_input and os.path.exists(video_path):
